...
def check_diagonal(x, y):
    ...
    four = 1
    for n in range(0, 4):
        if x + (n+1) > GRID_WIDTH or y + (n+1) > GRID_HEIGHT:
            one = 0
            break
        one *= grid[x + n][y + n]

    for n in range(0, 4):
        if x - (n+1) < 0 or y + (n+1) > GRID_HEIGHT:
            two = 0
            break
        two *= grid[x - n][y + n]

    for n in range(0, 4):
        if x - (n+1) < 0 or y - (n+1) < 0:
            three = 0
            break
        three *= grid[x - n][y - n]

    for n in range(0, 4):
        if x + (n+1) > GRID_WIDTH or y - (n+1) < 0:
            four = 0
            break
        four *= grid[x + n][y - n]
...
GRID_WIDTH = 20
GRID_HEIGHT = 20
grid = [[int(y) for y in x.split(" ")] for x in grid_str.split("\n")]
